...
{    
    ...
    for (UINT i = 0 ; i < paiMesh->mNumVertices ; i++) {
		...
		MyVertex v;
		v.vPos = XMFLOAT3(pPos->x, pPos->y, pPos->z);
		v.vNormal = XMFLOAT3(pNormal->x, pNormal->y, pNormal->z);
		v.vTexCoord = XMFLOAT2(pTexCoord->x, pTexCoord->y);
    ...
    for (UINT i = 0 ; i < pScene->mNumMaterials ; i++) {
        ...
        if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
            ...
            if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
				...
				//hr = D3DX11CreateShaderResourceViewFromFileA(pDevice, FullPath.c_str(),NULL, NULL, &pResource, &hr);


                ////hr = DXUTCreateShaderResourceViewFromFile(pDevice, FullPath.c_str(), NULL, NULL, &pResource, &hr);
                hr =DXUTCreateShaderResourceViewFromFile(pDevice, L"./models/Sphere/red.jpg", &pResource);
