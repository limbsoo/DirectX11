...
//					XMMATRIX mTemp = found->second->Offset;
...
//			dt +=timestep;
//			Calculate(dt);
//			Animations[i].Transforms.push_back(std::vector<XMMATRIX>());
//			std::vector<XMMATRIX>& trans = Animations[i].Transforms.back();
...
//	
//	XMMATRIX mTemp = internalNode->LocalTransform;
//	XMMatrixTranspose(mTemp);
