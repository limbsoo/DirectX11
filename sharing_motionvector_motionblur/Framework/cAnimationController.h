...
public:
	...
	std::vector<D3DXMATRIX>& GetTransforms(float dt){ return Transforms[GetFrameIndexAt(dt)]; }
	...
	std::vector<std::vector<D3DXMATRIX>> Transforms;//, QuatTransforms;/** Array to return transformations results inside. */
};
...
public:
	...
	D3DXMATRIX GetBoneTransform(float dt, const std::string& bname) { int bindex=GetBoneIndex(bname); if(bindex == -1) return D3DXMATRIX(); return Animations[CurrentAnimIndex].GetTransforms(dt)[bindex]; }
	// same as above, except takes the index
	D3DXMATRIX GetBoneTransform(float dt, unsigned int bindex) {  return Animations[CurrentAnimIndex].GetTransforms(dt)[bindex]; }
