...
#include "Graphics/WICTextureLoader11.h"
...
Texture::Texture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const std::string& filePath, aiTextureType type)
{
	...
	if (StringHelper::GetFileExtension(filePath) == ".dds")
	{
		...
		if (FAILED(hr))
		{
			this->Initialize1x1ColorTexture(device, N_Color::UnloadedTextureColor, type);
		}
		return;
	}
	else
	{
		HRESULT hr = DirectX::CreateWICTextureFromFile(device, deviceContext, StringHelper::StringToWide(filePath).c_str(), texture.GetAddressOf(), this->textureView.GetAddressOf(), 0);
		if (FAILED(hr))
		{
			this->Initialize1x1ColorTexture(device, N_Color::UnloadedTextureColor, type);
		}
