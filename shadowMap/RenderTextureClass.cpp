...
bool RenderTextureClass::Initialize(ID3D11Device* device, int textureWidth, int textureHeight, float SCREEN_NEAR, float SCREEN_FAR)
{
	...
	//m_viewport.Width = (float)800.f;
	//m_viewport.Height = (float)600.f;
	...
	// 투영 행렬을 설정합니다.
	m_projectionMatrix = XMMatrixPerspectiveFovLH(((float)XM_PI / 4.0f), ((float)textureWidth / (float)textureHeight), SCREEN_NEAR, SCREEN_FAR);

	// 2D 렌더링을 위한 직교 투영 행렬을 만듭니다.
	m_orthoMatrix = XMMatrixOrthographicLH((float)textureWidth, (float)textureHeight, SCREEN_NEAR, SCREEN_FAR);
